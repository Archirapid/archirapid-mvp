...
### 1. Jerarquía houseRoot/siteRoot (Líneas 611-643 babylon_editor.py)
...
```javascript
// ❌ ANTES: Guard bloqueaba updates tras normalización
...
```javascript
// ✅ DESPUÉS: Always overwrite — called after generateLayoutJS normalization
...
**Cambio principal**: Usar `getAbsolutePosition()` en lugar de `position` para coordenadas mundiales
...
### 6. Fix buildSolarPanels() (Idéntico patrón a buildRoof)

Mismo cambio: `fl.position` → `fl.getAbsolutePosition()` en líneas 2331-2335
...
2. Aunque el slider esté en 0, el roof se construye con offset actual

## Pendientes (Secondary Issues)

NO resueltos en esta fase, documentados para fase 2:
...
- [ ] Local: Abrir editor 3D
- [ ] Local: Mover sliders X/Z
- [ ] Local: Verificar tejado sigue la casa
- [ ] Local: Verificar paneles solares siguen
- [ ] Local: Cambiar voladizo mientras sliders activos
- [ ] Local: Cambiar dimensión de habitación (rebuildScene)
- [ ] Local: Verificar offset se mantiene tras rebuild
- [ ] Cloud: Deploy y confirm en Streamlit Cloud
